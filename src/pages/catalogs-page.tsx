      ...
      {showForm && (
        <div className="fixed inset-0 z-[70] grid place-items-center overflow-y-auto bg-slate-950/55 p-3 sm:p-4">
          <Card className="modal-panel p-5 sm:p-6" style={{ "--modal-width": "32rem" } as React.CSSProperties}>
            <div className="mb-5 flex items-start justify-between"><div><h2 className="text-lg font-bold">{editing ? "Editar valor" : "Nuevo valor"}</h2><p className="text-sm text-slate-500">{selectedDefinition.name}</p></div><button aria-label="Cerrar" onClick={() => setShowForm(false)}><X className="size-5 text-slate-400" /></button></div>
